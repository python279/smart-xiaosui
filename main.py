...
if __name__ == "__main__":
    ...
    # 初始化音频录制器
    audio_recorder = AudioRecorder(channels=1 if sys.platform == 'darwin' else 2)

    # 触发录音
    def _start_recording(audio_recorder: AudioRecorder, callback: Callable = None) -> None:
        if not audio_recorder.is_recording:
            threading.Thread(target=audio_recorder.start_recording, args=(callback,), daemon=True).start()
    ...
    def _audio_callback(audio_bytes: bytes) -> None:
        ...
        user_prompt = asr(audio_bytes)
        if user_prompt == "":
            return

        # 文字对话
        assistent_prompt = chat(system_prompt, user_prompt)

        # 文字转语音并播放
        tts = TTSClient(os.getenv("tts_app_id"),
                        os.getenv("tts_api_key"),
                        os.getenv("tts_api_secret"))
        tts(assistent_prompt, audio_player.play)
